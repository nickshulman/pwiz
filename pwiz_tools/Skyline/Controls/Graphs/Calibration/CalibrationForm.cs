...
namespace pwiz.Skyline.Controls.Graphs.Calibration
{
    public partial class CalibrationForm : DockableFormEx, IUpdatable
    {
        ...
        private void DisplayCalibrationCurve()
        {
            ...
            var mainPeptideQuantifier = curveFitter.PeptideListQuantifier;
            ...
            foreach (var sampleType in sampleTypes)
            {
                ...
                foreach (var standardIdentifier in curveFitter.EnumerateCalibrationPoints())
                {
                    ...
                    if (y.HasValue && x.HasValue)
                    {
                        ...
                        if (sampleType.AllowExclude && null == standardIdentifier.LabelType && samplePeptideQuantifier.IsExcludeFromCalibration(standardIdentifier.ReplicateIndex))
                        {
                ...
                {
                    curveLabel = QualifyCurveNameWithSurrogate(curveLabel, samplePeptideQuantifier.PeptideQuantifiers.Single().PeptideDocNode);
                }
