...
namespace pwiz.Skyline.Model.Results.Scoring
{
                ...
                foreach (var chromFileInfo in chromatogramSet.MSDataFileInfos)
                {
                    ...
                    while (summaryPeakData.NextPeakIndex())
                    {
                        ...
                        listRunFeatures.Add(new PeakGroupFeatures(peakIndex, peakTimes.RetentionTime, peakTimes.MedianTime,
                            peakTimes.StartTime, peakTimes.EndTime, peakAreasList, new FeatureScores(calcs.FeatureNames, features)));
                    }
    ...
    public struct PeakGroupFeatures
    {
        public PeakGroupFeatures(int peakIndex, float retentionTime,float medianRetentionTime, float startTime, float endTime,
            IList<float[]> peakAreasList, FeatureScores features) : this()
        {
