...
namespace pwiz.Skyline.Model.Results
{
    internal sealed class SpectraChromDataProvider : ChromDataProvider
    {
        ...
        private List<DigestedSpectrumMetadata> _spectrumMetadatas = new List<DigestedSpectrumMetadata>();
            ...
            while (_spectra.NextSpectrum())
            {
                ...
                else if (_filter.EnabledMsMs || _filter.EnabledMs)
                {
                    ...
                    else if (_filter.IsMsMsSpectrum(dataSpectrum))
                    {
                        ...
                        if (_demultiplexer == null)
                        {
                            ProcessSpectrumList(spectra, chromMap, rt, _filter, DigestedSpectrumMetadata.FromSpectrum(dataSpectrum));
                        }
                        else
                        {
                            ...
                            {
                                ProcessSpectrumList(new[] {deconvSpectrum}, chromMap, rt, _filter, DigestedSpectrumMetadata.FromSpectrum(dataSpectrum));
                            }
                        }
                    }
        ...
        private int GetScanIdIndex(DigestedSpectrumMetadata spectrumMetadata)
        {
        ...
        private int GetScanIdIndex(MsDataSpectrum spectrum)
        {
            ...
            return GetScanIdIndex(DigestedSpectrumMetadata.FromSpectrum(spectrum));
        }
        ...
        private void ProcessSpectrumList(MsDataSpectrum[] spectra,
                                     ...
                                     DigestedSpectrumMetadata spectrumMetadata)
        {
            lock (_blockWriter)
            {
