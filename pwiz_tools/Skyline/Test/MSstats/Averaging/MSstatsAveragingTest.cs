...
namespace pwiz.SkylineTest.MSstats.Averaging
{
    ...
    public class MSstatsAveragingTest : AbstractUnitTest
    {
        ...
        {
            var srmDocument = LoadRatPlasmaDocument();
            var documentSettings = new DocumentSettings(srmDocument, SettingsSnapshot.FromSettings(Settings.Default));
            var documentSettingsContainer =
                DocumentSettingsContainer.FromDocumentSettings(documentSettings, DataSchemaLocalizer.INVARIANT);
            var skylineDataSchema = new SkylineDataSchema(documentSettingsContainer);
            GroupComparisonModel model = new GroupComparisonModel(documentSettingsContainer.DocumentContainer, null);
            model.GroupComparisonDef = GroupComparisonDef.EMPTY.ChangeControlAnnotation("Condition")
                .ChangeControlValue("Healthy")
                .ChangeSummarizationMethod(SummarizationMethod.AVERAGING)
