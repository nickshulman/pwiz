...
namespace pwiz.SkylineTestData
{
    ...
    public class SpectrumSummaryTest : AbstractUnitTest
    {
        ...
        public void TestKdeAlignerToAlignmentFunction()
        {
            ...
            var points = Enumerable.Range(0, 100).Select(i => new PointPair(i, 0))
                .Concat(Enumerable.Range(0, 100).Select(i => new PointPair(100, i)))
                .Concat(Enumerable.Range(0, 100).Select(i => new PointPair(100 + i, 100)))
                .ToList();
            var heatMap = kdeAligner.GetHistogramAndTrain(points.Select(pt=>pt.X).ToArray(), points.Select(pt=>pt.Y).ToArray(), CancellationToken.None);
            ...
            for (int i = 0; i < 99; i++)
            {
                maxDifference = Math.Max(maxDifference, alignmentFunction.GetY(i));
            }
        ...
        private Bitmap GetAlignmentBitmap(KdeAligner aligner)
        {
            ...
            for (int x = 0; x < resolution; x++)
            {
                ...
                if (y >= 0 && y < resolution)
                {
                    ...
                    bitmap.SetPixel(x, resolution - y - 1, Color.Black);
                }
            }

            if (aligner.CanCalculateReverseRegression)
            {
                for (int y = 0; y < resolution; y++)
                {
                    var x = aligner.GetXCoordinate(aligner.GetValueReversed(aligner.GetScaledY(y)));
                    if (x >= 0 && x < resolution)
                    {
                        bitmap.SetPixel(x, resolution - y - 1, Color.Black);
                    }
